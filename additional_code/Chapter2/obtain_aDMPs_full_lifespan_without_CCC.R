...
##### In this version of the script, the control samples from the full human lifespan #####
...
# In our case with fit the following linear model: Beta~Age_years+Sex+PC1+...+PC17
...
calculate_aDMPs <- function(betas, metadata){
  ...
  pheno <- metadata;
  design <- model.matrix(~Age_years+Sex+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10+PC11+PC12+PC13+PC14+PC15+PC16+PC17, 
                         data=pheno);
...
write.table(x=dmps, file=paste0(output_path, '/aDMPs_final_full_lifespan_without_CCC.csv'), quote=F, sep=',', row.names=F);
